...
def fetch_entries(source: dict, max_items: int = 10) -> list[dict]:
    ...
    elif source.get("sde_gobierno"):
        return _fetch_sde_gobierno(source, max_items)
...
def _fetch_sde_gobierno(source: dict, max_items: int) -> list[dict]:
    """
    Scrapea el listado de noticias del Gobierno de Santiago del Estero (sde.gob.ar).
    Es un WordPress estándar: artículos en <article class="entry">,
    título en <h2 class="entry-title"><a>, imagen en primer <img>.
    """
    try:
        resp = requests.get(source["url"], headers=HEADERS, timeout=15)
        resp.raise_for_status()
        ...
        for art in articles:
            ...
            if not href or len(title) < 15 or href in seen:
                continue
            seen.add(href)
        ...
        logger.info(f"Gobierno SDE scraping: {len(entries)} artículos encontrados")
        return entries
    except Exception as e:
        logger.error(f"Gobierno SDE scraping error: {e}")
        return []
